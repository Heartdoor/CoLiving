...
public partial class FurnitureController : StaticBody2D
{
	...
	void Run() {
		...
		{
			inUse = true;
			var temp_texture = GetTexture2D($"res://FURNITURE/SPRITES/{furnitureData.name}_used.png");
		...
		{
			inUse = false;
			mySprite.Texture = GetTexture2D($"res://FURNITURE/SPRITES/{furnitureData.name}.png");
		}
	...
	void CreateAccessNode(AccessPosition position)
	{
		...
		myAccessNodes.Add(position, (Area2D)Add2DNode("res://FURNITURE/SCENES/access_node.tscn", this));
